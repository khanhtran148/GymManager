...
/// <summary>
/// Pure unit tests for GetRolesMetadataQueryHandler.
/// The handler is fully stateless (no DB, no DI deps), so these run without Docker/Testcontainers.
/// </summary>
public sealed class GetRolesMetadataQueryHandlerUnitTests
{
    private readonly GetRolesMetadataQueryHandler _handler = new();

    private async Task<RolesMetadataDto> GetMetadataAsync() =>
        (await _handler.Handle(new GetRolesMetadataQuery(), CancellationToken.None)).Value;
    ...
    [Fact]
    public async Task Roles_ContainsAllFiveRoles()
    {
        var dto = await GetMetadataAsync();

        dto.Roles.Should().HaveCount(5);
        dto.Roles.Select(r => r.Name)
            .Should().BeEquivalentTo(["Owner", "HouseManager", "Trainer", "Staff", "Member"]);
    }

    [Fact]
    public async Task Roles_OwnerIsNotAssignable()
    {
        var dto = await GetMetadataAsync();

        dto.Roles.Single(r => r.Name == "Owner").IsAssignable.Should().BeFalse();
    }
    ...
    [InlineData("Staff")]
    [InlineData("Member")]
    public async Task Roles_NonOwnerRolesAreAssignable(string roleName)
    {
        var dto = await GetMetadataAsync();

        dto.Roles.Single(r => r.Name == roleName).IsAssignable.Should().BeTrue();
    }

    [Fact]
    public async Task Roles_ValuesMatchEnumIntegers()
    {
        var dto = await GetMetadataAsync();

        dto.Roles.Should().Contain(r => r.Name == "Owner"        && r.Value == (int)Role.Owner);
        dto.Roles.Should().Contain(r => r.Name == "HouseManager" && r.Value == (int)Role.HouseManager);
        dto.Roles.Should().Contain(r => r.Name == "Trainer"      && r.Value == (int)Role.Trainer);
        dto.Roles.Should().Contain(r => r.Name == "Staff"        && r.Value == (int)Role.Staff);
        dto.Roles.Should().Contain(r => r.Name == "Member"       && r.Value == (int)Role.Member);
    }

    [Fact]
    public async Task Roles_AreOrderedByEnumValue()
    {
        var dto = await GetMetadataAsync();

        dto.Roles.Select(r => r.Value).Should().BeInAscendingOrder();
    }

    // ------------------------------------------------------------------ //
    // Permissions — count and exclusions
    // ------------------------------------------------------------------ //
    ...
    [Fact]
    public async Task Permissions_AreOrderedByBitPosition()
    {
        var dto = await GetMetadataAsync();

        dto.Permissions.Select(p => p.BitPosition).Should().BeInAscendingOrder();
    }

    // ------------------------------------------------------------------ //
    // Permissions — exact bit positions
    // ------------------------------------------------------------------ //
    ...
    public async Task Permissions_HasCorrectBitPosition(string permName, int expectedBit)
    {
        var dto = await GetMetadataAsync();

        dto.Permissions.Single(p => p.Name == permName).BitPosition.Should().Be(expectedBit);
    }
    ...
    public async Task Permissions_HasCorrectCategory(string permName, string expectedCategory)
    {
        var dto = await GetMetadataAsync();

        dto.Permissions.Single(p => p.Name == permName).Category.Should().Be(expectedCategory);
    }
    ...
    [Fact]
    public async Task RouteAccess_Returns17Rules()
    {
        var dto = await GetMetadataAsync();

        dto.RouteAccess.Should().HaveCount(17);
    }
    ...
    public async Task RouteAccess_RouteHasCorrectAllowedRoles(string path, string[] expectedRoles)
    {
        var dto = await GetMetadataAsync();

        var route = dto.RouteAccess.Single(r => r.Path == path);
