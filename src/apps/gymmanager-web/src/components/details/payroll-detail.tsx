...
export function PayrollDetail({ payrollId, onClose: _onClose }: PayrollDetailProps) {
  const { permissionMap } = useRbacStore();
  const { data: period, isLoading, error } = usePayrollPeriodById(payrollId);
  ...
  const canApprove = period.status === "Draft" || period.status === "PendingApproval";

  return (
    <div className="space-y-6">
      {/* Header row with period title and approve button */}
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <p className="text-xs font-semibold text-text-muted uppercase tracking-wider">Payroll Period</p>
